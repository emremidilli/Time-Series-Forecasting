...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.timeenc == 0:
            df_stamp['month'] = df_stamp\
                .date\
                .apply(lambda row: row.month, 1)
            df_stamp['day'] = df_stamp\
                .date\
                .apply(lambda row: row.day, 1)
            df_stamp['weekday'] = df_stamp\
                .date\
                .apply(lambda row: row.weekday(), 1)
            df_stamp['hour'] = df_stamp\
                .date\
                .apply(lambda row: row.hour, 1)
            data_stamp = df_stamp\
                .drop(['date'], axis=1)\
                .values
        elif self.timeenc == 1:
    ...
    def __read_data__(self):
        ...
        if self.timeenc == 0:
            ...
            df_stamp['hour'] = df_stamp\
                .date\
                .apply(lambda row: row.hour, 1)
            df_stamp['minute'] = df_stamp\
                .date\
                .apply(lambda row: row.minute, 1)
            df_stamp['minute'] = df_stamp\
                .minute\
                .map(lambda x: x // 15)
            data_stamp = df_stamp\
...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.timeenc == 0:
            df_stamp['month'] = df_stamp[self.time_col_name]\
                .apply(lambda row: row.month, 1)
            df_stamp['day'] = df_stamp[self.time_col_name]\
                .apply(lambda row: row.day, 1)
            df_stamp['weekday'] = df_stamp[self.time_col_name]\
                .apply(lambda row: row.weekday(), 1)
            df_stamp['hour'] = df_stamp[self.time_col_name]\
                .apply(lambda row: row.hour, 1)
            data_stamp = df_stamp\
                .drop([self.time_col_name], axis=1)\
                .values
        elif self.timeenc == 1:
            data_stamp = time_features(
