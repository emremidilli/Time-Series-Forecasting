...
def aGetCommonTimeStampsAccrossChannels():
    '''
    reads the channel files and identifies the common timestamps accross
        each channel.
    ...
    for sFileName in aFileNames:
        dfRaw = pd.read_csv(
            os.path.join(RAW_DATA_FOLDER, sFileName),
            delimiter='\t',
            usecols=['<DATE>', '<TIME>'])

        dfRaw.loc[:, 'TIME_STAMP'] = dfRaw.loc[:, '<DATE>'] + ' ' + \
            dfRaw.loc[:, '<TIME>']
        ...
        if arr is None:
            ...
            arr = np.intersect1d(arr, dfRaw.loc[:, 'TIME_STAMP'].to_numpy(
                np.datetime64))
...
def dfConvertToTimeSeriesDataset(aCommonTimeStamps, sFileName):
    '''
    identifies time stamps in way that they don't contain any gap within
        lookback and forecast horizons.
    in case a gap exists, that horizon is dropped.
    converts the dataset into a dataframe with the columns of [value],
        [group_id] and [time_idx]
    ...
    aLookbackTimeSteps = list(
        range(-(LOOKBACK_COEFFICIENT*FORECAST_HORIZON), 0))
    ...
    dfRaw.rename(
        columns={'<HIGH>': 'TICKER', '<VOL>': 'OBSERVED'},
        inplace=True)
    ...
    ix = pd.date_range(
        start=dfRaw.loc[:, 'TIME_STAMP'].min() + pd.Timedelta(
            f'{-1}{RAW_FREQUENCY}'),
        end=dfRaw.loc[:, 'TIME_STAMP'].max() + pd.Timedelta(
            f'{-1}{RAW_FREQUENCY}'),
        freq=f'{STEP_SIZE}{RAW_FREQUENCY}'
    )

    dfObserveds = pd.DataFrame(
        index=ix,
        columns=(aLookbackTimeSteps + aForecastTimeSteps),
        dtype='float64')
    dfLookbackTickers = pd.DataFrame(
        index=ix,
        columns=aLookbackTimeSteps,
        dtype='float64')
    ...
    for i in aLookbackTimeSteps:
        ...
        msk = np.isin(
            ixSearch.to_numpy(dtype=ix.dtype),
            dfFound.loc[:, 'TIME_STAMP'].to_numpy(dtype=ix.dtype))

        dfLookbackTickers.loc[msk, i] = dfFound.loc[:, 'TICKER'].to_numpy(
            dtype='float64')
        dfObserveds.loc[msk, i] = dfFound.loc[:, 'OBSERVED'].to_numpy(
            dtype='float64')

        dfTimeStamps.loc[msk, i] = dfFound.loc[:, 'TIME_STAMP'].to_numpy(
            ix.dtype)

    dfForecastTickers = pd.DataFrame(
        index=ix,
        columns=aForecastTimeSteps,
        dtype='float64')
    ...
    for i in aForecastTimeSteps:
        ...
        dfForecastTickers.loc[msk, i] = dfFound.loc[:, 'TICKER'].to_numpy(
            dtype='float64')
        dfObserveds.loc[msk, i] = dfFound.loc[:, 'OBSERVED'].to_numpy(
            dtype='float64')
    ...
    dfTickers = dfLookbackTickers.merge(
        right=dfForecastTickers,
        left_index=True,
        right_index=True,
        how='inner')

    aTimeIdx = np.reshape(
        dfTimeStamps.to_numpy(dtype=f'datetime64[{RAW_FREQUENCY_NUMPY}]'),
        (-1,)).astype(int)
    ...
    # tickers
    df = pd.DataFrame(
        data={'value': aTickers, 'group_id': 'ticker', 'time_idx': aTimeIdx})
    ...
    df = pd.DataFrame(
        data={
            'value': aObserveds, 'group_id': 'observed', 'time_idx': aTimeIdx
            })
    df.drop_duplicates(inplace=True, ignore_index=True)
    dfTsDataset = pd.concat([dfTsDataset, df])
...
if __name__ == '__main__':
    CONVERTED_DATA_FOLDER = os.path.join(
        os.environ.get('BIN_NAME'),
    ...
    if os.path.exists(CONVERTED_DATA_FOLDER) is True:
    ...
    for sFileName in aFileNames:
        ...
        dfTsDataset = dfConvertToTimeSeriesDataset(
            sFileName=sFileName,
            aCommonTimeStamps=aCommonTxs)
        dfTsDataset.to_csv(
            os.path.join(CONVERTED_DATA_FOLDER, f'{sFileName[:6]}.csv'),
            index=None,
            sep=';')
